...
const DOMAIN = 'https://rhyy.pre.suosishequ.com/'

// const DOMAIN   = `https://front.sit.suosihulian.com/`
...
const API_OCR  = `${DOMAIN}gateway/spider/ocr/ocr?accountType=1`


const SCAN_PATH= 'scan/screenshot.bmp'
...
export const getNameByOcr =async(img)=>{
  let filepath = path.join(process.cwd(), `scan/scan${dayjs().format('YYYMMDDHHmmss')}.bmp`)
  await saveIcon(img,filepath)

  const params = new FormData();
  params.append('multipartFile',  fs.createReadStream(filepath),'multipartFile.bmp')
  
  let s = await axios.post(API_OCR, params, {
    headers: {
...
export const saveIcon=async(pic, path)=> {
